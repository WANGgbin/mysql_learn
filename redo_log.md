...
写操作涉及页的变更，如果每次事务提交都同步数盘，会有大量随机 i/o 性能太差。

我们可以换一种思路来实现数据的持久化，那就是将数据的变更通过 log 文件记录下来，也就是所谓的 WAL，我们只要保证 log 不丢失，
那就可以基于 log 来恢复数据。

redo log 就是 mysql 的 WAL.

不过需要注意的是，即便有了 redo log，只能保证在单节点上数据不会丢失，分布式场景下，如果当前节点挂了，也没法重启，那数据还是会丢失。为此，会引入
主从同步，为了保证数据不丢失，我们还需要保证主从数据的一致性。所以还需要通过半复制方式，来保证事务提交时，消息已经在至少一个从节点上了。这样，即使
主节点挂了，数据还在从节点上，不会丢数据。
