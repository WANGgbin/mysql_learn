...
- 读写分离 & 写后读场景下，读不到写入的数据。
- 主备高可靠切换场景下，主备延迟，会影响服务的可用时间。
...
- gtid 生成
    ...
    怎么保证一个事务 id 在全局是唯一的呢？两种思路：
    - 由一个全局的 id 生成器生成 id
    - 各个节点按照相同的算法生成 id，比如经典的雪花算法。

mysql 中使用第二种算法来生成 gtid。gtid 包含两部分：source_uud:transition_no
source_uuid: 每个实例生成的唯一的 uuid
transition_no 每个实例维护的一个递增的序号，事务提交的时候，字段值 +1
    
- gtid 如何存储的

一个事务对应一个 gtid，而每个实例要维护大量的 gtid，是不是会耗费大量的空间？

实际上不是，大多数情况下，对于某个 source_uuid 而言，其分发的序号都是递增的，所以只需要维护一个区间信息即可，而不用维护所有的 gtid。

那怎么保证这些信息的持久化呢？通过表的方式存储。

mysql 中通过表 `mysql.gtid_executed` 来保存当前节点执行的所有 gtid. 其每行记录就是个 gtid 区间，格式如下：
```text
+--------------------------------------+----------------+--------------+----------+
| source_uuid                          | interval_start | interval_end | gtid_tag |
+--------------------------------------+----------------+--------------+----------+
```
  - source_uuid 表示对应某个实例
  - interval_start、interval_end 表示这个 gtid 区间
