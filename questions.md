...
4. 什么场景会生成大事务
   - 大量变更(大量更新、删除操作)
   - 业务代码在事务操作中带有其他耗时操作(rpc/api 调用)
   - client 接受 mysql 响应满，导致 mysql 线程处于 sending data to client(由于 socket send buf 满导致线程阻塞)，进而导致大事务。
   - 因占有锁而阻塞。
   - 由于 redo log 满或者 buffer pool 满导致 flush 脏页
5. delete 真正的删除数据了吗?
6. 如何查看慢查询日志? https://blog.csdn.net/m0_67394002/article/details/126113577
7. sql 都有哪些数据类型? 以及使用场景是什么?
8. 加锁相关纠正
